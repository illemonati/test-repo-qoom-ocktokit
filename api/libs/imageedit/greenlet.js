...
export default function greenlet(asyncFunction) {
    ...
    const script = '$$=' + asyncFunction + ';onmessage=' + (e => {
        ...
        Promise.resolve(e.data[1]).then(v => $$.apply($$, v)).then(
        // success handler - callback(id, SUCCESS(0), result)
        // if `d` is transferable transfer zero-copy
        d => {
            postMessage([e.data[0], 0, d], [d].filter(x => ((x instanceof ArrayBuffer) ||
    ...
    return function (args) {
        ...
        return new Promise(function () {
            ...
            worker.postMessage([currentId, args], args.filter(x => ((x instanceof ArrayBuffer) ||
                (x instanceof MessagePort) ||
                (self.ImageBitmap && x instanceof ImageBitmap))));
        });
    };
